...
public class ContractRepositoryModel : PageModel
{
    ...
    private readonly ILogger<ContractRepositoryModel> _logger;

    public ContractRepositoryModel(ILogger<ContractRepositoryModel> logger)
    {
        _logger = logger;
    }
    ...
    public void OnGet()
    {
        ...
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(builded.ConnectionString))
            {
                ...
                //get all templates that are not archived and in alphabetical order
                String sql = "Select templateName, version, dateCreated, filePathName, notes FROM ContractTemplate WHERE isArchived = 0 ORDER BY templateName ASC";
